...
const FreePlanDailog: React.FC<FreePlanDialogProps> = ({ open, onClose }) => {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-gray-900 bg-opacity-30 px-2 sm:px-4">
      <div className="bg-white rounded-lg shadow-xl w-full max-w-4xl p-4 sm:p-6 md:p-8 relative mx-auto overflow-y-auto max-h-[95vh]">
        {/* Header */}
        <div className="flex items-center justify-between mb-4 sm:mb-6">
          <h2 className="text-lg sm:text-2xl font-semibold text-[#5A8DB8]">Basic Profile Preview</h2>
          <button
            className="text-gray-400 hover:text-[#5A8DB8] text-2xl font-bold absolute top-4 right-4 sm:top-6 sm:right-8"
            onClick={onClose}
        ...
        <div className="flex flex-col md:flex-row gap-4 sm:gap-8">
          ...
          <div className="flex flex-col items-center md:w-1/3 w-full">
            <div className="w-32 h-40 sm:w-48 sm:h-56 rounded-lg overflow-hidden mb-4 border border-gray-200">
              <img src={profileImg} alt="Profile" className="object-cover w-full h-full" />
            </div>
            <div className="bg-gray-100 rounded-lg p-3 sm:p-4 text-center text-xs sm:text-sm mb-4">
            ...
            <div className="bg-white border rounded-lg p-3 sm:p-4 text-center w-full">
          ...
          <div className="flex-1 flex flex-col gap-3 sm:gap-4">
            <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-2">
              ...
              <div className="bg-[#E6F0FA] border border-[#5A8DB8] rounded-lg p-3 sm:p-4 flex flex-col items-center w-full sm:w-48 mt-2 sm:mt-0">
            ...
            <div>
              ...
              <div className="flex items-center mt-1">
                <input
                  ...
                  value={profileUrl}
                  readOnly
                  className="border border-[#5A8DB8] rounded-l px-2 py-1 w-full text-xs sm:text-sm bg-gray-50 focus:ring-2 focus:ring-[#5A8DB8]"
                />
                <button
                  className={`bg-[#5A8DB8] px-3 py-1 rounded-r text-xs sm:text-sm hover:bg-[#3C5979] ml-1 text-white font-semibold ${copied ? 'bg-green-600' : ''}`}
                  onClick={handleCopy}
                  type="button"
                >
                  {copied ? 'Copied!' : 'copy'}
            ...
            <div>
              ...
              <div className="text-xs text-[#5A8DB8] mt-1 cursor-pointer">Show all services →</div>
            </div>
            {/* Skills */}
            <div>
              ...
              <div className="text-xs text-[#5A8DB8] mt-1 cursor-pointer">Show all skills →</div>
            </div>
            {/* Tools */}
            <div>
            ...
            <div className="flex flex-col sm:flex-row justify-end gap-2 sm:gap-4 mt-4 sm:mt-6">
              <button
                className="px-4 sm:px-6 py-2 rounded border border-[#5A8DB8] bg-white text-[#5A8DB8] hover:bg-[#E6F0FA] text-xs sm:text-base font-semibold"
                onClick={onClose}
                type="button"
              >
                Cancel
              </button>
              <button className="px-4 sm:px-6 py-2 rounded bg-[#5A8DB8] text-white font-semibold hover:bg-[#3C5979] text-xs sm:text-base" onClick={() => navigate('/free-plan')}>Continue to Checkout</button>
            </div>
          </div>
        </div>
      </div>
